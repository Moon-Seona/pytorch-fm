...
#        pred_seq, hidden = self.autoencoder(additional[:,:149])
        ...
        feat_embedding = torch.cat([feat_embedding[:,:4,:], encoded2, feat_embedding[:,4:,:]], dim=1)
        ...
        return torch.sigmoid(x.squeeze(1)), pred_seq2, hidden2, encoded2.squeeze(1), feat_embedding[:,4,:]
